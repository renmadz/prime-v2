...
| **Status (2026-07-09, Phase 15 investigation)** | **Confirmed still required — kept as a known limitation, not resolved.** Re-ran the full backend suite without `--no-file-parallelism`: it still fails, this time in `export.test.ts`'s `afterAll` cleanup (`proposal_versions_form_template_version_id_fkey` violation racing against another file's shared `formTemplateVersion` fixture teardown) — a different symptom from the originally-logged `formTemplates.test.ts` / `proposals.test.ts` / `submission.test.ts` collisions, but the same root cause: multiple test files sharing one physical Postgres test DB, racing on `deleteMany` during parallel teardown of overlapping fixture rows. A real fix (per-worker DB/schema isolation, or wrapping each test in a rolled-back transaction) is a test-infrastructure rework, judged out of scope for a security-hardening/regression phase. `apps/backend/package.json`'s `test` and `test:local` scripts already run `--no-file-parallelism` and were left unchanged; the full suite is 132/132 green under that mode. **Contingency unchanged: do not re-enable parallel test execution.** Re-evaluate the real fix if/when backend test suite runtime (currently ~63s sequential) becomes a bottleneck. |
