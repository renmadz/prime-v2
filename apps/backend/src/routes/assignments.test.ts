import { afterAll, beforeAll, describe, expect, it } from "vitest";
import type { FastifyInstance } from "fastify";
import bcrypt from "bcryptjs";
import { PrismaClient } from "@prisma/client";
import { buildApp } from "../app.js";
import { ROLE_CODES } from "../utils/roles.js";

// ── Env setup ────────────────────────────────────────────────────────────────

const TEST_DATABASE_URL =
  process.env.TEST_DATABASE_URL ??
  "postgresql://primev2_user:devpassword123@localhost:5433/primev2_test";

process.env.NODE_ENV = "development";
process.env.DATABASE_URL = TEST_DATABASE_URL;
process.env.SESSION_SECRET = "a".repeat(64);
process.env.MINIO_ENDPOINT = "localhost:9000";
process.env.MINIO_ACCESS_KEY = "test-access-key";
process.env.MINIO_SECRET_KEY = "test-secret-key";
process.env.MINIO_BUCKET_NAME = "test-bucket";
process.env.GOOGLE_CLIENT_ID = "test-client-id";
process.env.GOOGLE_CLIENT_SECRET = "test-client-secret";
process.env.GOOGLE_CALLBACK_URL = "http://localhost:3000/api/auth/google/callback";
process.env.FRONTEND_URL = "http://localhost:5173";
process.env.API_URL = "http://localhost:3000";

const db = new PrismaClient({ datasources: { db: { url: TEST_DATABASE_URL } } });

// ── Helpers ──────────────────────────────────────────────────────────────────

function sessionCookieHeader(response: { cookies: Array<{ name: string; value: string }> }) {
  const cookie = response.cookies.find((c) => c.name === "sessionId");
  return cookie ? `sessionId=${cookie.value}` : "";
}

let ipCounter = 700;
function nextIp() {
  ipCounter += 1;
  return `10.0.9.${ipCounter}`;
}

async function ensureRolesSeeded() {
  for (const code of ROLE_CODES) {
    await db.role.upsert({
      where: { code },
      update: {},
      create: { code, name: code, isActive: true },
    });
  }
}

async function createUserWithRole(email: string, password: string, roleCode: string) {
  const passwordHash = await bcrypt.hash(password, 12);
  const role = await db.role.findUniqueOrThrow({ where: { code: roleCode } });
  return db.user.create({
    data: {
      email,
      passwordHash,
      firstName: "Test",
      lastName: roleCode,
      isActive: true,
      mustChangePassword: false,
      userRoles: { create: [{ roleId: role.id }] },
    },
  });
}

async function loginStaff(app: FastifyInstance, email: string, password: string) {
  const response = await app.inject({
    method: "POST",
    url: "/api/auth/staff/login",
    remoteAddress: nextIp(),
    payload: { email, password },
  });
  if (response.statusCode !== 200) {
    throw new Error(
      `Failed to create staff session for ${email}: ${response.statusCode} ${response.body}`,
    );
  }
  return sessionCookieHeader(response);
}

// ── Test identifiers for cleanup ─────────────────────────────────────────────

const ADMIN_EMAIL = "asgn-admin@test.local";
const FOCAL_EMAIL = "asgn-focal@test.local";
const NON_ADMIN_EMAIL = "asgn-non-admin@test.local";
const APPLICANT_EMAIL = "asgn-applicant@test.local";
const PASSWORD = "AsgnTestPassw0rd!";
const TEST_EMAILS = [ADMIN_EMAIL, FOCAL_EMAIL, NON_ADMIN_EMAIL, APPLICANT_EMAIL];

const TEST_PROPOSAL_TYPE_CODE = "PT-ASGN-TEST-01";
const TEST_OFFICE_CODE = "OFFICE-ASGN-TEST-01";
const TEST_PROGRAM_CODE = "PROG-ASGN-TEST-01";
const TEST_FORM_CODE = "FT-ASGN-TEST-01";

async function cleanupTestData() {
  const users = await db.user.findMany({ where: { email: { in: TEST_EMAILS } } });
  const userIds = users.map((u) => u.id);

  const proposalType = await db.proposalType.findUnique({
    where: { code: TEST_PROPOSAL_TYPE_CODE },
  });

  if (proposalType) {
    const proposals = await db.proposal.findMany({ where: { proposalTypeId: proposalType.id } });
    const proposalIds = proposals.map((p) => p.id);

    if (proposalIds.length > 0) {
      await db.proposalAssignment.deleteMany({ where: { proposalId: { in: proposalIds } } });
      const versions = await db.proposalVersion.findMany({
        where: { proposalId: { in: proposalIds } },
      });
      const versionIds = versions.map((v) => v.id);
      if (versionIds.length > 0) {
        await db.proposalFieldValue.deleteMany({ where: { proposalVersionId: { in: versionIds } } });
      }
      await db.proposal.updateMany({ where: { id: { in: proposalIds } }, data: { currentVersionId: null } });
      await db.proposalVersion.deleteMany({ where: { proposalId: { in: proposalIds } } });
      await db.proposal.deleteMany({ where: { id: { in: proposalIds } } });
    }

    await db.proposalType.update({ where: { id: proposalType.id }, data: { defaultFormTemplateId: null } });
    await db.proposalType.delete({ where: { id: proposalType.id } });
  }

  const formTemplate = await db.formTemplate.findUnique({ where: { formCode: TEST_FORM_CODE } });
  if (formTemplate) {
    const versions = await db.formTemplateVersion.findMany({ where: { formTemplateId: formTemplate.id } });
    const versionIds = versions.map((v) => v.id);
    if (versionIds.length > 0) {
      const sections = await db.formSection.findMany({ where: { formTemplateVersionId: { in: versionIds } } });
      const sectionIds = sections.map((s) => s.id);
      if (sectionIds.length > 0) {
        await db.formField.deleteMany({ where: { formSectionId: { in: sectionIds } } });
        await db.formSection.deleteMany({ where: { id: { in: sectionIds } } });
      }
      await db.formTemplateVersion.deleteMany({ where: { id: { in: versionIds } } });
    }
    await db.formTemplate.delete({ where: { id: formTemplate.id } });
  }

  const program = await db.program.findUnique({ where: { code: TEST_PROGRAM_CODE } });
  if (program) await db.program.delete({ where: { id: program.id } });

  const office = await db.office.findUnique({ where: { code: TEST_OFFICE_CODE } });
  if (office) await db.office.delete({ where: { id: office.id } });

  if (userIds.length > 0) {
    await db.auditLog.deleteMany({ where: { actorUserId: { in: userIds } } });
    await db.userRole.deleteMany({ where: { userId: { in: userIds } } });
    await db.staffProfile.deleteMany({ where: { userId: { in: userIds } } });
    await db.applicantProfile.deleteMany({ where: { userId: { in: userIds } } });
    await db.user.deleteMany({ where: { id: { in: userIds } } });
  }
}

// ── Test suite ────────────────────────────────────────────────────────────────

describe("Assignments routes (Phase 21A)", () => {
  let app: FastifyInstance;
  let adminUserId: string;
  let focalUserId: string;
  let proposalId: string;
  let adminCookie: string;
  let nonAdminCookie: string;

  beforeAll(async () => {
    await ensureRolesSeeded();
    await cleanupTestData();
    app = await buildApp();
    await app.ready();

    const office = await db.office.create({
      data: { code: TEST_OFFICE_CODE, name: "Test Office (Assignments)" },
    });
    const program = await db.program.create({
      data: { code: TEST_PROGRAM_CODE, name: "Test Program (Assignments)", officeId: office.id },
    });
    const formTemplate = await db.formTemplate.create({
      data: { formCode: TEST_FORM_CODE, title: "Test Assignments Form", isActive: true },
    });
    const formVersion = await db.formTemplateVersion.create({
      data: {
        formTemplateId: formTemplate.id,
        versionNumber: 1,
        schemaVersion: "1.0",
        isCurrent: true,
        publishedAt: new Date(),
      },
    });
    const proposalType = await db.proposalType.create({
      data: {
        code: TEST_PROPOSAL_TYPE_CODE,
        name: "Test Assignments Grant",
        programId: program.id,
        defaultFormTemplateId: formTemplate.id,
        isActive: true,
      },
    });

    const applicantUser = await createUserWithRole(APPLICANT_EMAIL, PASSWORD, "APPLICANT");
    const proposal = await db.proposal.create({
      data: {
        applicantUserId: applicantUser.id,
        proposalTypeId: proposalType.id,
        title: "Assignments Test Proposal",
        status: "SUBMITTED_TO_FOCAL",
      },
    });
    proposalId = proposal.id;
    void formVersion;

    const adminUser = await createUserWithRole(ADMIN_EMAIL, PASSWORD, "ADMIN");
    adminUserId = adminUser.id;
    const focalUser = await createUserWithRole(FOCAL_EMAIL, PASSWORD, "PROJECT_FOCAL");
    focalUserId = focalUser.id;
    await createUserWithRole(NON_ADMIN_EMAIL, PASSWORD, "PROJECT_FOCAL");

    adminCookie = await loginStaff(app, ADMIN_EMAIL, PASSWORD);
    nonAdminCookie = await loginStaff(app, NON_ADMIN_EMAIL, PASSWORD);
  });

  afterAll(async () => {
    await cleanupTestData();
    await app.close();
    await db.$disconnect();
  });

  it("TC-ASGN-01: POST as ADMIN creates an assignment → 201", async () => {
    const response = await app.inject({
      method: "POST",
      url: `/api/proposals/${proposalId}/assignments`,
      headers: { cookie: adminCookie },
      payload: { userId: focalUserId, roleCode: "PROJECT_FOCAL" },
    });
    expect(response.statusCode).toBe(201);
    const body = response.json();
    expect(body.userId).toBe(focalUserId);
    expect(body.roleCode).toBe("PROJECT_FOCAL");
    expect(body.isActive).toBe(true);
    expect(body.user.email).toBe(FOCAL_EMAIL);
  });

  it("TC-ASGN-02: POST as non-ADMIN returns 403", async () => {
    const response = await app.inject({
      method: "POST",
      url: `/api/proposals/${proposalId}/assignments`,
      headers: { cookie: nonAdminCookie },
      payload: { userId: focalUserId, roleCode: "PROJECT_FOCAL" },
    });
    expect(response.statusCode).toBe(403);
  });

  it("TC-ASGN-03: POST again for same proposal/user/role reactivates instead of duplicating → 200", async () => {
    const response = await app.inject({
      method: "POST",
      url: `/api/proposals/${proposalId}/assignments`,
      headers: { cookie: adminCookie },
      payload: { userId: focalUserId, roleCode: "PROJECT_FOCAL" },
    });
    expect(response.statusCode).toBe(200);

    const rows = await db.proposalAssignment.findMany({
      where: { proposalId, userId: focalUserId, roleCode: "PROJECT_FOCAL" },
    });
    expect(rows).toHaveLength(1);
  });

  it("TC-ASGN-04: GET lists active assignments for the proposal", async () => {
    const response = await app.inject({
      method: "GET",
      url: `/api/proposals/${proposalId}/assignments`,
      headers: { cookie: adminCookie },
    });
    expect(response.statusCode).toBe(200);
    const body = response.json();
    expect(Array.isArray(body)).toBe(true);
    expect(body.some((a: { userId: string }) => a.userId === focalUserId)).toBe(true);
  });

  it("TC-ASGN-05: DELETE soft-deactivates the assignment", async () => {
    const listResponse = await app.inject({
      method: "GET",
      url: `/api/proposals/${proposalId}/assignments`,
      headers: { cookie: adminCookie },
    });
    const [assignment] = listResponse.json();

    const deleteResponse = await app.inject({
      method: "DELETE",
      url: `/api/proposals/${proposalId}/assignments/${assignment.id}`,
      headers: { cookie: adminCookie },
    });
    expect(deleteResponse.statusCode).toBe(200);
    expect(deleteResponse.json().isActive).toBe(false);

    const row = await db.proposalAssignment.findUniqueOrThrow({ where: { id: assignment.id } });
    expect(row.isActive).toBe(false);

    const listAfter = await app.inject({
      method: "GET",
      url: `/api/proposals/${proposalId}/assignments`,
      headers: { cookie: adminCookie },
    });
    expect(listAfter.json()).toHaveLength(0);
  });

  it("TC-ASGN-06: POST/GET/DELETE without auth returns 401", async () => {
    const post = await app.inject({
      method: "POST",
      url: `/api/proposals/${proposalId}/assignments`,
      payload: { userId: focalUserId, roleCode: "PROJECT_FOCAL" },
    });
    expect(post.statusCode).toBe(401);

    const get = await app.inject({ method: "GET", url: `/api/proposals/${proposalId}/assignments` });
    expect(get.statusCode).toBe(401);
  });

  it("TC-ASGN-07: POST with unknown userId returns 422", async () => {
    const response = await app.inject({
      method: "POST",
      url: `/api/proposals/${proposalId}/assignments`,
      headers: { cookie: adminCookie },
      payload: { userId: "00000000-0000-0000-0000-000000000000", roleCode: "PROJECT_FOCAL" },
    });
    expect(response.statusCode).toBe(422);
  });

  it("TC-ASGN-08: POST to unknown proposal returns 404", async () => {
    const response = await app.inject({
      method: "POST",
      url: "/api/proposals/00000000-0000-0000-0000-000000000000/assignments",
      headers: { cookie: adminCookie },
      payload: { userId: focalUserId, roleCode: "PROJECT_FOCAL" },
    });
    expect(response.statusCode).toBe(404);
  });

  void adminUserId;
});
